...
#+begin_src elisp
  ...
  (defun tcp-server-filter (process string)
    "tcp 处理消息"
    (message "收到消息: %s" string)
    (process-send-string process (format "服务器收到")))
        ...
        (progn
          (setq server-process
                (make-network-process
                 :name "tcp-server"                 ;; 进程的名字
                 :service 8080                      ;; 端口号
                 :host "127.0.0.1"                  ;; ip 地址
                 :server t                          ;; 创建服务器进程
                 :family 'ipv4                      ;; 是用 ipv4
                 :noquery t                         ;; Emacs 退出时自动终止进程
                 :filter 'tcp-server-filter         ;; 处理消息
                 :sentinel 'server-sentinel))       ;; 处理连接状态
                (message "TCP 服务器已启动在端口 %d" 8080))
  ...
  (defun udp-server ()
    ...
    (if (null server-process)
        (progn
          (setq server-process
                (make-network-process
                 :name "udp-server"                 ;; 进程的名字
                 :service 8080                      ;; 端口号
                 :host "127.0.0.1"                  ;; ip 地址
                 :family 'ipv4                      ;; 是用 ipv4
                 :server t                          ;; 创建服务器进程
                 :type 'datagram                    ;; UDP 协议
                 :noquery t                         ;; Emacs 退出时自动终止进程
                 :filter 'udp-server-filter))       ;; 处理消息
                (message "UDP 服务器已启动在端口 %d" 8080))
      (progn (delete-process server-process)
             (setq server-process nil)
             (message "UDP 服务器已停止"))))
#+end_src
